...
export class CheckoutComponent implements OnInit {
  ...
  copyShippingAddressToBillingAddress(event: any) {
    if (event.target.checked) {
      ...
      // bug fix for states, when state is not copied
  ...
  onSubmit() {
    ...
    const customerControl = this.checkoutFormGroup.get('customer');

    if (customerControl && customerControl.value) {
      console.log(customerControl.value);
      console.log("The email address is " + customerControl.value.email);
    } else {
      console.error("Customer form control or its value is null or undefined.");
    }
